...
void Actor_LoadSpawnTable(u16* spawn_table) {
    u16 actor_index;
    u16 jndex;
    u16 index;
    u16 counter;
    ...
    jndex = 0;
    counter = 0;
    while (spawn_table[0] != 0xFF00) {
        if (!(spawn_table[0] & 0x2000)) {
            actor_index = func_8004398C(spawn_table);

            if (spawn_table[0] & 0x8000) {
                D_800D357C[index + 0] = actor_index;
                D_800D357C[index + 1] = gActors[actor_index].actorType;
                index += 2;
            }

            if (spawn_table[0] & 0x1000) {
    ...
    for (; index < 0x40; index += 2) {
        D_800D357C[(s32)index] = 0;
    }
    for (; jndex < 0x60; jndex += 3) {
        D_800D361C[(s32)jndex] = 0;
    }
}
